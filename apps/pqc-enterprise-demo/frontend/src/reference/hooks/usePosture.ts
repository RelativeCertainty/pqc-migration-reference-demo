...
export function parseRuntimePosture(value: unknown): RuntimePosture {
  ...
  if (value.schema_version !== "pqc-posture.v3" || value.evidence_tier !== "request_transport_observation") {
    throw new Error("unexpected posture contract");
  }
  ...
  if (!isClosedObject(value.access_control, ["state", "target"]) ||
      value.access_control.state !== "synthetic_session_only" || value.access_control.target !== "C# assessment-scoped demo authorization") {
    throw new Error("unexpected posture contract");
  }
  ...
  if (!isClosedObject(value.data_posture, ["storage", "upstream_origin_dependency", "dataset", "request_logging"]) ||
      value.data_posture.storage !== "isolated_sqlite" || value.data_posture.upstream_origin_dependency !== "none" ||
